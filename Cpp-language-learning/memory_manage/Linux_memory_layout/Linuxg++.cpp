...
int main()
{
    ...
    int *pp_i = (int *) malloc(sizeof(int) * 20000); // int指针指向malloc开辟的int空间
    ...
    printf("%08x : local char pointer\n", &str);
    printf("%08x : local int\n", &l_i);
    printf("%08x : local const int\n", &c);
    ...
    printf("%08x : global char pointer point content g_p2\n", g_p2);
    printf("%08x : global char pointer point content g_p\n", g_p);
    printf("%08x : local char pointer point content\n", str);


    printf("%08x : static function addresss\n", s_fun);
